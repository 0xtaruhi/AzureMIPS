...
case class DCache(config: CoreConfig = CoreConfig()) extends Component {
  ...
  for (i <- 0 until dcachecfg.portNum) {
    for(j <- 0 until dcachecfg.wayNum) {
      hit_bits(i)(j) := valids(i)(j) && (tags_for_match(i)(j) === ptags(j))
      miss_bits(i)(j) := valids(i)(j) && (tags_for_match(i)(j) =/= ptags(j))
    }
  }
  ...
  for(i <- 0 until dcachecfg.portNum) {
    val bank_offset = getBankOffset(dreq_cut_pkg12(i).paddr) 
  ...
  when (!stall_12) {
    which_dreq23 := which_dreq23_nxt
    which_ram23 := which_ram23_nxt
  }
  ...
  val fsm_arbiter = new StateMachine {
    ...
    val IDLE : State = new State with EntryPoint {
      whenIsActive {
        (has_mshr_waitings zip mshrs).foreach { case(h, m) => h := m.fsm_mshr.isActive(x.WAIT_CBUS)}
  ...
  val mshrs = for(i <- 0 until dcachecfg.portNum) yield {
    new Area {
      ...
      val mshr_creq = CReq()
      val cbus_occupy = (fsm_arbiter.has_mshr_waitings.orR && io.cresp.last && mshrs(fsm_arbiter.mshr_id).fsm_mshr.isActive(LOAD)) || 
                          (fsm_arbiter.isEntering(BUSY) && mshr_id_nxt === U(i))
