...
class ReadRegfile extends Component {
  val io = new Bundle {
    ...
    val exBypass  = Vec(in(new ExBypassPort), 2)
    ...
    val wbBypass   = Vec(in(new BypassPort), 2)
  ...
  when (io.exBypass.map(_.hit(io.decodedSignals.op1Addr)).reduce(_ || _)) {
    ...
    io.prevStall := Mux(io.exBypass.map(_.stall(io.decodedSignals.op1Addr)).reduce(_ || _), True, False)
  } elsewhen (io.mem1Bypass.map(_.hit(io.decodedSignals.op1Addr)).reduce(_ || _)) {
    io.readrfSignals.op1Data := 0
    io.prevStall := True
  ...
  } elsewhen (io.wbBypass.map(_.hit(io.decodedSignals.op1Addr)).reduce(_ || _)) {
    io.readrfSignals.op1Data := io.wbBypass.map{ bp => Mux(bp.hit(io.decodedSignals.op1Addr), bp.wrData, U(0)) }.reduce(_ | _)
  } otherwise {
   ...
   when (io.exBypass.map(_.hit(io.decodedSignals.op2Addr)).reduce(_ || _)) {
    ...
    io.prevStall := Mux(io.exBypass.map(_.stall(io.decodedSignals.op2Addr)).reduce(_ || _), True, False)
  } elsewhen (io.mem1Bypass.map(_.hit(io.decodedSignals.op2Addr)).reduce(_ || _)) {
    io.readrfSignals.op2Data := 0
    io.prevStall := True
    ...
    io.readrfSignals.op2Data := io.generalRegfile(0).data
  }
